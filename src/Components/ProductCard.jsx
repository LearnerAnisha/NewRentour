import { Link } from "react-router-dom";
import React, { useEffect, useState } from "react";
import axiosInstance from "../utils/axiosInstance";

const ProductCard = ({ product }) => {
    const [fetchedProduct, setFetchedProduct] = useState(null);
    
    const baseUrl = import.meta.env.VITE_API_BASE_URL;

    

    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const res = await axiosInstance.get(`${baseUrl}/api/category/${product.item_category}/${product.item_id}/`);
                console.log("Fetched product in ProductCard:", res.data);
                setFetchedProduct(res.data);
            } catch (err) {
                console.error("Error fetching product in ProductCard:", err);
            }
        };


        if (product?.item_id) {
            fetchProduct();
        }
    }, [product?.item_id]);

    const displayProduct = fetchedProduct || product;

    return (
        <div className="relative group w-[90%] sm:min-w-[22%] sm:w-[300px] bg-white rounded-lg h-[55dvh] shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden">
            
            {/* Badge */}
            {displayProduct.badge && (
                <div className="absolute top-2 left-2 bg-black text-white text-xs font-bold px-2 py-1 rounded">
                    {displayProduct.badge}
                </div>
            )}

            {/* Image section */}
            <div className="h-[80%] aspect-auto bg-gray-100 w-full p-[2px]">
                <Link to={`/product/${displayProduct.item_category}/${displayProduct.item_id}`} className="w-full h-full aspect-auto">
                    <img
                        src={`${baseUrl}${displayProduct.productAvatar}`}
                        alt={displayProduct.item_name}
                        onError={(e) => {
                            e.currentTarget.src = "/fallback.png";
                            e.currentTarget.onerror = null;
                        }}
                        className="w-full h-full object-contain"
                    />
                </Link>
            </div>

            {/* Product Info */}
            <div className="p-2 h-[20%] text-center">
                <Link to={`/${displayProduct.item_category}/${displayProduct.item_id}`}>
                    <h2 className="font-semibold text-black text-lg group-hover:text-gray-800 transition-colors duration-300">
                        {displayProduct.item_name}
                    </h2>
                </Link>
                <div className="flex items-center justify-center gap-x-4">
                    <span className="text-red-500 line-through">₹{displayProduct.final_item_price}</span>
                </div>
            </div>
        </div>
    );
};

export default ProductCard;
